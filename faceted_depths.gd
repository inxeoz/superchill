...
func load_level(index: int) -> void:
	...
	elif level_name == "FACETED DEPTHS":
...
func campaign_is_final() -> bool:
	# The sandbox is a standalone test bench, not the next chapter. The run ends
	# at the last real level before it instead of dropping the player into it.
	if level_theme == "sandbox":
		return true
	for later in range(level_index + 1, LEVELS.size()):
		if String(LEVELS[later].get("theme", "")) != "sandbox":
			return false
	return true
