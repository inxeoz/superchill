...
### Crafting Table
- **Crafting table:** `B` opens a square overlay on Level 0. The unified ELEMENT SOURCES list starts empty and, as items are collected, reveals them as normal craft elements in a two-column grid — bottle sources first, then debris kinds (LEAVES, PLASTIC WRAPPERS, ROPE, WOOD SCRAPS, COILED SPRINGS). `W/S` selects among the discovered elements, `Space` adds an available item to one of eight jacket slots, `X` removes the last slot, `Enter` combines a complete set of eight items — bottles, debris, or any mix — and `B/Esc` closes. The life jacket is an ordinary inventory item that happens to be wearable: the status line reports WEARING (G to drop) or ON GROUND (E to wear). A dropped life jacket can also be equipped from this overlay with `E` when nearby.
